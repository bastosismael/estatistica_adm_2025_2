...
1) No que consiste o processo de Inferência Estatística? Podemos realizar inferência quando trabalhamos com uma amostra oriunda de um processo de amostragem não probabilístico?
\vspace{5px}
...
\begin{enumerate}[a)] % a), b), c), ...
    ...
    \item $\sum\limits_{i = 1}^4 (-1)^{i}$ \\
    \item $\sum\limits_{i = 1}^3 4x$ \\
    \item $\sum\limits_{i = 1}^n (-1)^{i}$ onde $n$ é par\\
    \item $\sum\limits_{i = 1}^n (-1)^{i}$ onde $n$ é ímpar\\
    \item $\prod\limits_{i=1}^5 (i-1)^2$\\
    \item $\prod\limits_{i=1}^3 2^i$\\
    \item $\prod\limits_{i=1}^n -1$ onde $n$ é par\\
    \item $\sum\limits_{x \in \{ 1, 2,5,6,20,10\}} x $ \\ 
    \item $\sum\limits_{s \in \{ 2,4,6,8,10\}}  \dfrac{e^s}{\sum\limits_{m \in \{ 2,4,6,8,10\}} e^m} $ \\
    \item $\sum\limits_{i=1}^5\sum\limits_{j=1}^3 2^j$
    \item $\sum\limits_{i=1}^5\sum\limits_{j=0}^i 3^{(j-1)}$
\end{enumerate}
\vspace{5px}
...
\begin{enumerate}[a)]
    \item Calcule a média da Idade dos participantes. Você acha que fornecer apenas a média é suficiente para entender o comportamento dessa variável? Justifique.
    \item Construa uma tabela de frequência para a variável Curso. A variável Curso é qualitativa nominal ou ordinal? E a variável Nome?
