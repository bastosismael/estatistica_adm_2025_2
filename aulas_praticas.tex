...
\title{Estatística para Administração - Aulas Práticas}
...
\begin{frame}{Operações envolvendo vetores}
  As mesmas operações que vimos para números funcionam também para vetores desde que ambos 
...
\begin{frame}{Tarefa 3 - Lidando com vetores no R}
  ...
  \begin{exercicio}
    \begin{enumerate}[a)]
      \item Crie um vetor com o nome que você quiser e armazene o nome de 5 pessoas da turma.
      \item Acesse o elemento que ocupa a segunda posição do vetor do item a).
      \item Acesse os elementos que ocupam a segunda e primeira posição do vetor do item a).
      \item Crie um vetor com o nome que quiser e armazene as notas de cada aluno. 
      \item Multiplique o vetor criado no item d) por 3. 
      \item Calcule a raiz quadrada dos elementos do vetor do item d). 
    \end{enumerate}
  \end{exercicio}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Como ler um arquivo Excel no R\\RStudio}
  Existem duas formas de ler um arquivo Excel no RStudio. 
  A primeira é através de uma função:

  \pause
  \begin{lstlisting}[language=R]
  dados = read.csv("Caminho do arquivo")
  \end{lstlisting}
\end{frame}

\begin{frame}
  Segunda forma: Lendo pela interface do RStudio. 
\end{frame}

\begin{frame}{Como a tabela é armazenada dentro do R?}
  \begin{itemize}
    \item Uma tabela é armazenada dentro do R como uma estrutura chamada DataFrame
    \pause
    \item Para visualizar a tabela no RStudio, basta usar o camando \textit{View}.
    \pause
    \item Para visualizar todas as variáveis (colunas) da tabela, usamos o comando \textit{names}.
    \pause
    \item As colunas de um DataFrame são tratadas como vetores, logo, podemos aplicar todo o conhecimento de vetores que vimos anteriormente.
    \pause
    \item Para acessar uma coluna, basta usar o nome que atribuimos à tabela seguido pelo símbolo \$ e o nome da coluna. Dica, apertar tab depois do \$ exibe as colunas possíveis.
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{Filtrando uma tabela}
Para filtrar o conjunto de dados baseado em uma condição, primeiro devemos escrever o nome do DataFrame acompanhado da condição dentro de colchetes
 ...
 \begin{exemplo}
  Suponha que tenhamos interesse em filtrar, no nosso conjunto de dados, todos os indivíduos que tenham idade inferior a 45 anos. O código para isso seria:
...
\frametitle{Filtrando uma tabela}
\begin{exemplo}
Podemos também querer filtrar basedo em um conjunto de valores, nesse caso usamos o comparador lógico \%in\%.

\begin{lstlisting}[language=R]
  dados = read.csv(file.choose()) # Lendo a base de dados do curso.
  dados_l45 = dados[dados$Idade %in% c(12, 45, 27, 30), ] # Coluna Ocupacao contendo apenas indivíduos com idade inferior a 45 anos.
\end{lstlisting}
\end{exemplo}
\end{frame}

\begin{frame}{Tarefa 4 - Lidando com arquivos Excel}
  \begin{enumerate}[a)]
    \item Leia a base de dados (tabela) do curso dentro do RStudio.
    \item Visualize a tabela dentro do RStudio.
    \item Mostre na tela os valores da coluna Ocupacao.
    \item Qual o tamanho (número de linhas) dessa tabela?
    \item Calcule a média, mediana, variância e desvio padrão dos valores da variável  Duracao do Sono.
    \item Crie uma tabela contendo a frequência nominal para a variável Sexo.
    \item Crie uma tabela contendo a frequência relativa para a variável Ocupacao.   
    \item Considere que essa tabela é a população. Obtenha uma amostra aleatória simples de tamanho 50. 
    \item Desafio: Construa um boxplot para a variável Nivel de atividade física.
    \item Desafio: Construa um boxplot para a variável Nivel de atividade física por sexo.
\end{enumerate}
\end{frame}

\begin{frame}[fragile]
\frametitle{Lendo arquivos Excel no formato xls ou xlsx}

Para ler arquivos Excel, primeiro precisamos instalar a biblioteca \textit{readxl}.
...
\begin{itemize}
  \item Após isso, lemos o arquivo usando o comando \textit{read\_excel}.
  \item Podemos também ler pela interface do RStudio. 
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{Amostragem Aleatória Estratificada no R}
