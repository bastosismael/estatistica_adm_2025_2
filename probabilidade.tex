...
\begin{frame}{Operações envolvendo eventos}
    ...
    \begin{enumerate}[a)] % a), b), c), ...
        ...
        \pause
        \item Se $A_1, \dots, A_n$ for qualquer coleção finita de eventos, então $\displaystyle\bigcup_{i=1}^n A_i$ será o evento que ocorrerá se, e somente se, ao menos um dos eventos $A_i$ ocorrer. 
        \pause
        \item Se $A_1, \dots, A_n$ for qualquer coleção finita de eventos, então $\displaystyle\bigcap_{i=1}^n A_i$ será o evento que ocorrerá se, e somente se, todos os eventos $A_i$ ocorrer. 
    \end{enumerate}
\end{frame}
...
\begin{frame}{Probabilidade - Intepretação Frequentista}
    \begin{exemplo}
...
\begin{frame}{Sempre podemos calcular probabilidades?}
    \begin{exemplo}
...
\begin{frame}{Regra da multiplicação}
 ...
 \begin{atencao}
    Note que a regra da multiplicação é uma consequência direta da definição de probabilidade condicional. 
 \end{atencao}
\end{frame}
...
%     \pause
%     \item Denotaremos por $\Omega_X$ o conjunto de possíveis valores assumidos pela Variável Aleatória $X$. Dizemos que  $\Omega_X$ é suporte de $X$. 
%     \pause
